 ...
 * Po ukončení sedenia extrahuje z konverzácie JSON profil používateľa cez Claude Haiku.
 * Frontend si profil ukladá do localStorage a posiela ho pri ďalšom /api/chat.
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return res.status(502).json({ error: "Extrakcia zlyhala." });
    }
    ...
    // Claude niekedy obalí JSON do ```json … ``` — vyčisti
